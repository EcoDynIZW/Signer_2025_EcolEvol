...
env_var <- stack(paste0(dir_proc, "/env_vars_100m.tif"))
...
dat_tracks_ssf <- 
  readr::read_rds(paste0(dir_est, "/ssf.rds")) |> 
  mutate(method = "iSSA") |> 
  unnest(fits) |> 
  filter(case_ == FALSE) |> 
...
dat_tracks_rp <- 
  readr::read_rds(paste0(dir_est, "/rsf.rds")) |> 
  mutate(method = "RSA") |> 
  unnest(data) |> 
  filter(ID_sim  == 1) |> 
  dplyr::select(method, id, res, x, y, hab, dtw, imp)

## combine data sets
dat_all <- 
  dat_tracks_gps |> 
  bind_rows(dat_tracks_ssf) |> 
  bind_rows(dat_tracks_rp) |> 
  mutate(
    method = factor(method, levels = c("GPS data", "SSA-rs", "RSA-rp")),
...
rm(dat_tracks_gps, dat_tracks_ssf, dat_tracks_rp)
...
ggsave(paste0(dir_plots, "/distributions_emp_res30_wide.pdf"),
       width = 10, height = 5.8, device = cairo_pdf)
ggsave(paste0(dir_ms, "/s13_distributions_emp_res30_wide.png"),
...
ggsave(paste0(dir_plots, "/distributions_emp_res360_wide.pdf"),
       width = 10, height = 5.8, device = cairo_pdf)
ggsave(paste0(dir_ms, "/s14_distributions_emp_res360_wide.png"),
       width = 10, height = 5.8, dpi = 600, bg = "white")
```
...
ggsave(paste0(dir_plots, "/distributions_emp_res1440_wide.pdf"),
       width = 10, height = 5.8, device = cairo_pdf)
ggsave(paste0(dir_ms, "/s15_distributions_emp_res1440_wide.png"),
       width = 10, height = 5.8, dpi = 600, bg = "white")
```
