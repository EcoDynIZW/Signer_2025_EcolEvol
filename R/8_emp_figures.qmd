...
#| fig-width: 10.5
#| fig-height: 11.5
dat_est_ssf <- read_rds(paste0(dir_est, "/ssf_est.rds")) |>
  dplyr::select(id, res, est) |>
  mutate(run_id = 1:nrow(pick(id))) |>
  unnest(cols = est) |> mutate(method = "issf")

dat_est_rsf <- read_rds(paste0(dir_est, "/rsf_est.rds")) |>
  dplyr::select(id, res, est) |>
  mutate(run_id = 1:nrow(pick(id))) |>
...
dat_tracks_ssf <- 
  readr::read_rds(paste0(dir_est, "/ssf_fits.rds")) |> 
  unnest(fits) |> 
  mutate(method = "iSSA") |> 
  filter(case_ == FALSE) |> 
...
dat_tracks_rsf <- 
  readr::read_rds(paste0(dir_est, "/rsf_fits.rds")) |> 
  mutate(method = "RSA") |> 
  unnest(fits) |> 
  filter(case_ == FALSE) |> 
  group_by(id, res) |> 
